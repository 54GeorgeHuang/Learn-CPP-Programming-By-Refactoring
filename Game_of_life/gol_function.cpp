...
void read_cells(bool cells[MAXSIZE][MAXSIZE], const int grid_size)
{
	for (int row = 0; row < grid_size; row++) 
		for (int col = 0; col < grid_size; col++) {
			...
			std::cin >> temp;
			cells[row][col] = (temp == 1)? true: false;
		}
}

int numbers_of_live_neighbors(const bool cells[MAXSIZE][MAXSIZE], const int now_row, const int now_col, const int grid_size)
{
...
void print_cells(const bool cells[MAXSIZE][MAXSIZE], const int grid_size, const int iter = -1)
{
...
int main(void)
{
	...
	// 2: 2 states
	bool cells[2][MAXSIZE][MAXSIZE];

	read_cells(cells[old_state], grid_size);
	print_cells(cells[old_state], grid_size);

	for (int iter = 0; iter < generations; iter++) {
		evolve(cells[old_state], cells[new_state], grid_size);
		print_cells(cells[new_state], grid_size, iter);
